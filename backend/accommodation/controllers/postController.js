...
exports.getUserAccommodationPosts = (req, res) => {
  ...
  db.query(userSql, [userId], (userErr, userResult) => {
    ...
    const postSql = `
      SELECT
          ...
          (
            SELECT COUNT(*)
            FROM accommodation_post_likes jl
            WHERE jl. accommodation_post_id = jp.id AND jl.like = 1
          ) AS total_likes,
 
          -- Total Comments
          (
            SELECT COUNT(*)
            FROM accommodation_post_comments jc
            WHERE jc. accommodation_post_id = jp.id
          ) AS total_comments,
          ...
          (
            ...
            WHERE c. accommodation_post_id = jp.id
          ) AS comments_data
 
      FROM  accommodation_post jp
    ...
    db.query(postSql, [userId], (postErr, postResults) => {
      ...
      res.status(200).json({
        message: "User + accommodation posts fetched successfully",
