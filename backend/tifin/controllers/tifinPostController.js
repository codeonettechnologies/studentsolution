...
exports.searchTiffinPosts = async (req, res) => {
  try {
    ...
    // 1️⃣ Fetch main tiffin posts with user info
    const sql = `
      SELECT 
        tp.*, 
    ...
    db.query(sql, [searchValue, searchValue], (err, posts) => {
      ...
      // 2️⃣ Fetch likes data (who liked)
      const likesSql = `
      ...
      // 3️⃣ Fetch comments data (who commented + comment text)
      const commentsSql = `
      ...
      db.query(likesSql, [postIds], (likeErr, likes) => {
        ...
        db.query(commentsSql, [postIds], (commentErr, comments) => {
          ...
          // 4️⃣ Combine everything together
