...
exports.getUserCoachingPosts = (req, res) => {
  ...
  db.query(userSql, [userId], (userErr, userResult) => {
    ...
    const postSql = `
      SELECT
          ...
          (
            SELECT COUNT(*)
            FROM coaching_post_likes jl
            WHERE jl. coaching_post_id = jp.id AND jl.like = 1
          ) AS total_likes,
 
          -- Total Comments
          (
            SELECT COUNT(*)
            FROM coaching_post_comment jc
            WHERE jc. coaching_post_id = jp.id
          ) AS total_comments,
          ...
          (
            ...
            WHERE c. coaching_post_id = jp.id
          ) AS comments_data
 
      FROM  coaching_post jp
    ...
    db.query(postSql, [userId], (postErr, postResults) => {
      ...
      res.status(200).json({
        message: "User + coaching posts fetched successfully",
        user: user, 
        posts: postResults,
      });
    });
